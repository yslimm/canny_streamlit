...
if upload_file is not None:
    ...
    st.image(img_gray,
             ...
             use_column_width=True)
    # 02. Gaussian Blur
    kernel = st.sidebar.slider('Blur kernel size', min_value= 0, max_value=10, value= 7 )
    kernelb = (kernel * 2) + 1
    ...
    canny_param_dict = {'blur kernel_size ': kernel,
    ...
    img_Download = st.sidebar.button(label='download final image')

    if img_Download:
        st.sidebar.write('Thanks')
        cv2.imwrite(filename='saved/trimmed-001.png', img= img_61)
    else:
        st.sidebar.write('nothing happend')
